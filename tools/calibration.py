...
def ACE(data, bin_num, correct_fn=None, vis=True, save_pth='reliability_diagrams', prefix='', testing=False):
    '''
    data: [
        [
            word_confidence, 
    ...
    for i,(s, t, c, a) in enumerate(zip(min_p_bin, total_bin, correct_bin, acc_bin)):
        if i!=bin_num-1:
            record_str += f'|{s:0.3f}~{min_p_bin[i+1]:0.3f}|{int(t):7d}|{int(c):7d}|{a:0.3f}|\n'
        else:
            record_str += f'|{s:0.3f}~{1.:0.3f}|{int(t):7d}|{int(c):7d}|{a:0.3f}|\n'
    ...
    record_str += f'ACC:{ACC:0.5f}\n'
    record_str += f'ECE:{ECE:0.5f}\n'
    record_str += f'BrierScore:{Brier:0.5f}\n'
    ...
    if testing: print(record_str + flag_str)
    if vis:
        #print(record_str)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_dir', type=str, default='exp/', help='Where to load json file')
    parser.add_argument('--rd_output_dir', type=str, default='./Reliability_Diagram')
    parser.add_argument('--save_path', type=str, default='results1.xlsx')
