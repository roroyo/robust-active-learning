...
from dataset_al import hierarchical_dataset, AlignCollate
...
def benchmark_all_eval(model, criterion, converter, opt, calculate_infer_time=False):
    ...
    eval_data_list = ['IIIT5k_3000', 'SVT', 'ic03_867', 'IC13_857',
    ...
    for eval_data in eval_data_list:
        ...
        print(f'Acc {accuracy_by_best_model:0.3f}\t normalized_ED {norm_ED_by_best_model:0.5f}')
        log.write(f'Acc {accuracy_by_best_model:0.3f}\t normalized_ED {norm_ED_by_best_model:0.5f}\n')
        print(dashed_line)
        log.write(dashed_line + '\n')
...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    preds_data = []
    for i, (image_tensors, labels, _, _,_) in enumerate(evaluation_loader):
...
def test(opt):
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            current_ece, acc, _, pzhECE, flag_str = ACE(preds_data, bin_num=15, vis=True, correct_fn=None, testing=True)
